...
const registerShortcut = async (shortcut?: string) => {
  try {
    // 获取当前配置的快捷键
    const shortcutKey =
      shortcut || (await getLocalShortcut("showMainWinHotKey", "option+v", "alt+v"))
    // 注册快捷键
...
export const initialization = async () => {
  try {
    ...
    // 检查是否为静默启动
    const silentStart = await Storage.get(SETTING_STORAGE_KEYS.SILENT_START, false)
    if (!silentStart) {
      // 启动后显示剪切板窗口,不跟随鼠标位置
      await invoke("show_clipboard_window", { mousePosition: false })
    }
  } catch (error) {
...
const initializeOnboarding = async () => {
  const onboarding = await Storage.get(SETTING_STORAGE_KEYS.IS_FIRST_START, true)
  if (onboarding) {
